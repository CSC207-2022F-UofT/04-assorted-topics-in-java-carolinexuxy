...
class DrivableMap {
    ...
    public List<Tradable> getTradable(){
        /* arrayList that will store the Tradable items fround in drivable_map*/
        List<Tradable> tradableObject = new ArrayList<Tradable>();

        for (Drivable object: this.drivable_map.values()) {
            if (object instanceof Tradable) {
                tradableObject.add((Tradable) object);
            }
        }
